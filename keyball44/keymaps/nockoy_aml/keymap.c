...
bool scroll_mode_toggle = false;    // スクロールモードのトグル状態 Toggle state of scroll mode
...
int16_t scroll_v_threshold = 10;    // この閾値を超える度に垂直スクロールが実行される。 Vertical scrolling is performed each time this threshold is exceeded.
int16_t scroll_h_threshold = 10;    // この閾値を超える度に水平スクロールが実行される。 Each time this threshold is exceeded, horizontal scrolling is performed.
...
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
    ...
    switch (keycode) {
        ...
        case KC_MY_SCR:
            if (record->event.pressed) {
                scroll_mode_toggle = !scroll_mode_toggle;  // トグル切り替え Toggle switch
                if (scroll_mode_toggle) {
                    state = SCROLLING;
                } else {
                    enable_click_layer();   // スクロールモードOFF時はクリックレイヤーを有効にする Enable click layer when scroll mode is OFF
                }
            }
         return false;
        ...
        case KC_SCROLL_DIR_H:
         ...
         default:
            if  (record->event.pressed) {
                ...
                if (state == CLICKING || (state == SCROLLING && !scroll_mode_toggle))
                {
                    enable_click_layer();
                    return false;
                }
                else if (state == SCROLLING && scroll_mode_toggle)
                {
                    // スクロールモードがトグルONの時は状態を維持 Maintain state when scroll mode toggle is ON
                    return false;
                }
...
report_mouse_t pointing_device_task_user(report_mouse_t mouse_report) {
    ...
    // スクロールモードがトグルONの場合は常にSCROLLING状態にする If scroll mode toggle is ON, always set to SCROLLING state
    if (scroll_mode_toggle && state != SCROLLING) {
        state = SCROLLING;
        enable_click_layer();
    }
    // スクロールモードがトグルOFFの場合はSCROLLING状態から抜ける Exit SCROLLING state when scroll mode toggle is OFF
    else if (!scroll_mode_toggle && state == SCROLLING) {
        enable_click_layer();
    }
    ...
    if (current_x != 0 || current_y != 0) {
        ...
        switch (state) {
            ...
            case SCROLLING:
            {
                ...
                current_h = -rep_h / scroll_h_threshold;  // 水平スクロール方向を逆転 Reverse horizontal scroll direction
                current_v = rep_v / scroll_v_threshold;   // 垂直スクロール方向を逆転 Reverse vertical scroll direction
                current_x = 0;
                current_y = 0;
            }
                break;
    ...
    else
    {
        switch (state) {
            ...
            case CLICKABLE:
                if (timer_elapsed(click_timer) > to_reset_time && !scroll_mode_toggle) {
                    disable_click_layer();
                }
                break;
